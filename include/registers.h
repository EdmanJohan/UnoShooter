...
#define PIC32_USTA_URXISEL \
    0x000000C0 /* Bitmask: receive interrupt is set when... */
#define PIC32_USTA_URXISEL_NEMP 0x00000000 /* ...receive buffer is not empty \
                                              */
#define PIC32_USTA_URXISEL_HALF \
    0x00000040 /* ...receive buffer becomes 1/2 full */
#define PIC32_USTA_URXISEL_3_4 \
    0x00000080 /* ...receive buffer becomes 3/4 full */
#define PIC32_USTA_TRMT \
    0x00000100 /* Transmit shift register is empty (read-only) */
...
#define PIC32_USTA_UTXISEL \
    0x0000C000 /* Bitmask: TX interrupt is generated when... */
#define PIC32_USTA_UTXISEL_1 \
    0x00000000 /* ...the transmit buffer contains at least one empty space */
#define PIC32_USTA_UTXISEL_ALL \
    0x00004000 /* ...all characters have been transmitted */
#define PIC32_USTA_UTXISEL_EMP \
    0x00008000                       /* ...the transmit buffer becomes empty */
...
#define PIC32_PMCON_ADRMUX_AD \
    0x0800 /* Lower address on PMD[7:0], high on PMA[15:8] */
 ...
 * PMP Mode register.
 */
#define PIC32_PMMODE_WAITE(x) \
    ((x) << 0) /* Wait states: data hold after RW strobe */
#define PIC32_PMMODE_WAITM(x) ((x) << 2) /* Wait states: data RW strobe */
#define PIC32_PMMODE_WAITB(x) \
    ((x) << 6)                   /* Wait states: data setup to RW strobe */
...
#define PIC32_SPICON_CKE                       \
    0x00000100 /* Output data changes on       \
                * transition from active clock \
                * state to Idle clock state */
#define PIC32_SPICON_SMP                                                  \
    0x00000200                         /* Master mode: input data sampled \
...
#define PIC32_SPICON_SPIFE                                            \
    0x00020000                         /* Frame synchronization pulse \
                                        * coincides with the first bit clock */
#define PIC32_SPICON_FRMPOL 0x20000000 /* Frame pulse is active-high */
#define PIC32_SPICON_FRMSYNC \
    0x40000000                        /* Frame sync pulse input (Slave mode) */
...
#define PIC32_DEVCFG(cfg0, cfg1, cfg2, cfg3)                  \
    asm(".section .config");                                  \
    unsigned __DEVCFG0 __attribute__((section(".config0"))) = \
        (cfg0) ^ 0x7fffffff;                                  \
    unsigned __DEVCFG1 __attribute__((section(".config1"))) = \
        (cfg1) | DEVCFG1_UNUSED;                              \
    unsigned __DEVCFG2 __attribute__((section(".config2"))) = \
        (cfg2) | DEVCFG2_UNUSED;                              \
    unsigned __DEVCFG3 __attribute__((section(".config3"))) = \
        (cfg3) | DEVCFG3_UNUSED
...
#define DEVCFG1_FNOSC_FRCDIVPLL \
    0x00000001                          /* Fast RC with divide-by-N and PLL */
 ...
 */
#define PIC32_INTCON_INT0EP \
    0x0001 /* External interrupt 0 polarity rising edge */
#define PIC32_INTCON_INT1EP \
    0x0002 /* External interrupt 1 polarity rising edge */
#define PIC32_INTCON_INT2EP \
    0x0004 /* External interrupt 2 polarity rising edge */
#define PIC32_INTCON_INT3EP \
    0x0008 /* External interrupt 3 polarity rising edge */
#define PIC32_INTCON_INT4EP \
    0x0010 /* External interrupt 4 polarity rising edge */

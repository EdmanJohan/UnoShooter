...
void update_display() {
    ...
    for (x = 0; x < OLED_PAGE_MAX; x++) {
        ...
        for (y = 0; y < 16; y++) {
            ...
            for (k = 0; k < 8; k++) spi_put_byte(font[c * 8 + k]);
        }
    }
}
...
void print_string(int line, char *string) {
    ...
    if (line < 0 || line >= 4 || !string) return;
...
void print_image(int line, const byte *data) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        spi_put_byte(i);     // SET MEM ADDRESSING MODE.
                             // 0b00 = HORI, 0b01 = VERT, 0b10 =
                             // PAGE, 0b11 = INVALID
...
void remove_image(int line, const byte *data) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        for (j = 0; j < 32; j++) spi_put_byte(255);
    }
}
